...
- [His Own Mints](project_loopring_own_mints.md) - account 33443 + all 13 of his mints with nftIDs, derived from calldata; archive state; and the 784-candidate reconstruction that failed. **Blocker LIFTED 2026-08-10: the builder was validated 8/8 against real Loopring metadata, so a miss implicates the name/description bytes, not the pipeline.** Do not re-derive. **Collections identified: `0x8eb42287...` = 3D Metaverse Assets (owner-stated, no on-chain name exists); `0x7a520803...` UNVERIFIED, do not guess.**
