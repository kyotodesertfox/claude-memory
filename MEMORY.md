...
- [Loopring Protocol / Decode Reference](project_loopring_protocol.md) — decode reference for L1 block calldata, cross-checked against protocol source AND live mainnet blocks. **Key fact: the published repo source does NOT match what was deployed for NFT mints — deployed blocks carry nftID inline and contain zero NFT_DATA transactions. When source and live data conflict, live data wins.** Per-type layouts labelled by how each was verified, plus the creatorFeeBips 1-byte fix, the Float24 transfer fix, why zero addresses are correct, and the token-decimals accuracy trap. **2026-08-09 contract sweep: NFT_DATA (type 9) DOES exist on mainnet - 82,314 txs carrying collection+minter+creatorFeeBips+nftType, so collections resolve from calldata; NFT_MINT byte 1 is mintType not nftType (live bug in derive-from-calldata.js); royalties are not protocol-enforced anywhere; smart wallet EIP-1271 is just the owner's signature.**
- [Loopring nftID](project_loopring_nftid.md) — **what `nftID` is and why it is the join key across all three layers: the L1 calldata mint record, the `CIDv0(nft_id)` metadata address, and any snapshot/claim registry entry.** A Merkle root proves inclusion and can never reveal omission; calldata reconstruction is the only thing that makes omission visible, which moves evidentiary authority from a party to a public process. Holds the canonical nftID<->CID conversion, "nftID addresses the METADATA not the image", THE VERIFIER IS THE nftID (not IPFS), positioning, and the **flagged unresolved conflict** over whether the pipeline was ever validated against a real nftID. Structural claim only - do not convert to intent.
- [Calldata Retrieval](loopring_calldata_retrieval.md) — **state of the L1 sweep: 67,870 / 67,886 fetched, every block sha256-verified, zero failures.** NOT complete and the script says so: 16 txs found-but-not-stored (listed by hash, still queued, a re-run picks them up) and 186 L1 blocks never scanned (23569814..23569999) which means the DENOMINATOR is incomplete. **Derived tables are stale** - nft_mints/nft_collections were built at ~14k blocks and span only 57,923..67,894, so ~44,000 collected blocks are undecoded and any absence they report is meaningless until re-derived. Commands: status / verify / enumerate / fetch / all.
- [Loopring Recovery](project_loopring_recovery.md) — **THE JOB: present a JSON whose hash matches the nftID. Provide the bytes, get the hash. Nothing is being fixed or reconstructed - the creator already holds the bytes.** The format is CRACKED AND BYTE-VERIFIED (2026-08-03): flat JSON, keys alphabetically sorted, CRLF endings, no indent; only the separator (1-2 CRLF) and trailing whitespace (0-3 CRLF) vary, so it is 8 deterministic candidates, never a search. Also: nftID addresses the METADATA, not the image (why hashing an image never matches, now confirmed on-chain via uri()); the SDK conversion is canonical with a test vector; CREATE2 is verified as a formula but resolves nothing (0/12 vs ground truth); collections come from L1 TransferSingle logs; the "smart wallets only" claim was false. **THE VERIFIER IS THE nftID, NOT IPFS - retrieval tests availability, never correctness. Our hash matching Loopring's client has NEVER been demonstrated; only that it matches the IPFS reference spec.**
- [His Own Mints](project_loopring_own_mints.md) - account 33443 + all 13 of his mints with nftIDs, derived from calldata; archive state; and the 784-candidate reconstruction that failed. **The blocker: the JSON builder has never been validated against a known-good sample, so every miss is uninterpretable.** Do not re-derive. **Collections identified: `0x8eb42287...` = 3D Metaverse Assets (owner-stated, no on-chain name exists); `0x7a520803...` UNVERIFIED, do not guess.**
