...
- [Loopring Recovery](project_loopring_recovery.md) — what is provably rebuildable (provenance: fully, from calldata alone, demonstrated) vs conditional (content). **The SDK (`~/github/lonewolf-loopring/loopring_sdk`) settles the addressing question: nftID↔CID is Loopring's own canonical implementation with a committed test vector, and nftID addresses the METADATA JSON, not the image — which is why hashing an image against nftID never matches.** Also: CREATE2 salt bakes in the baseURI (counterfactual collections are ownership-provable), the offline metadata-reconstruction path, and the corrected "smart wallets only" claim (false — EOAs resolve fine).
